...
impl TmHeaderValidate {
    pub fn run(input: &[u8], output: &mut BytesRef) -> Result<(), &'static str> {
        ...
        TmHeaderValidate::header_matches_commit(&header.signed_header.as_ref().unwrap())?;
    ...
    fn validator_sets_match(light_block: &LightBlock) -> Result<(), &'static str> {
        let validators_hash =
            TmHeaderValidate::hash_validator_set(&light_block.validator_set.as_ref().unwrap());
    ...
    fn next_validators_match(light_block: &LightBlock) -> Result<(), &'static str> {
        let next_validators_hash =
            TmHeaderValidate::hash_validator_set(&light_block.next_validator_set.as_ref().unwrap());
    ...
    fn verify_sufficient_validators_overlap(
        ...
        let non_absent_votes = signatures.iter().enumerate().flat_map(|(idx, signature)| {
            if let Some(vote) = TmHeaderValidate::non_absent_vote(
                signature,
                idx,
    ...
    fn non_absent_vote(
        ...
        let p = parts::Header {
            ...
            hash: thash::Sha256(ph),
        };
        let prost_value = prost_types::Timestamp {
            seconds: timestamp.as_ref().unwrap().seconds,
            nanos: timestamp.as_ref().unwrap().nanos,
        };
        let time = SystemTime::try_from(prost_value).unwrap().into();
        Some(Vote {
            vote_type: Precommit,
            height: (commit_sig.height as u64).try_into().unwrap(),
            round: (commit_sig.round as u32).try_into().unwrap(),
            block_id: Some(Id {
                hash: thash::Sha256(h),
                part_set_header: p,
            }),
            timestamp: Some(time),
            validator_address: validator_address.clone().try_into().unwrap(),
